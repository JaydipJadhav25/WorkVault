...
const PrivateRoute = ({ children, roles = [], requireCompleteProfile = true } : {children : any, roles  : any , requireCompleteProfile : any }) => {
  ...
  if (requireCompleteProfile && user && !(user as any).isProfileComplete && location.pathname !== '/complete-profile') {
    return <Navigate to="/complete-profile" replace />;
  }

  // Check role-based access
  if (roles.length > 0 && user && !roles.includes(!(user as any).role)) {
